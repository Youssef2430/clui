 ...
 *
 * This is a GLUI-owned permission broker that approximates Claude Code's
 ...
 *   - Per-run settings files (only GLUI-spawned sessions see the hook)
 */
...
export class PermissionServer extends EventEmitter {
  ...
  generateSettingsFile(runToken: string, env?: Record<string, string>): string {
    ...
    const dir = join(tmpdir(), 'glui-hook-config')
    try { mkdirSync(dir, { recursive: true, mode: 0o700 }) } catch {}

    const filePath = join(dir, `glui-hook-${runToken}.json`)
