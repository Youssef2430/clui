...
export function useAgentEvents() {
  ...
  useEffect(() => {
    ...
    const unsubSnapshot = window.glui.onThreadSnapshot((tabId, snapshot) => useSessionStore.getState().applyThreadSnapshot(tabId, snapshot))

    const unsubEvent = window.glui.onEvent((tabId, event) => {
    ...
    const unsubStatus = window.glui.onTabStatusChange((tabId, newStatus, oldStatus) => {
      if (rafIdRef.current) { cancelAnimationFrame(rafIdRef.current); flushChunks() }
      handleStatusChange(tabId, newStatus, oldStatus)
    })

    const unsubError = window.glui.onError((tabId, error) => {
      handleError(tabId, error)
    })

    const unsubBtw = window.glui.onBtwEvent((evt) => {
    ...
    const unsubSkill = window.glui.onSkillStatus((status) => {
      if (status.state === 'failed') {
        console.warn(`[GLUI] Skill install failed: ${status.name} — ${status.error}`)
      }
    })
  ...
  // Note: window.glui.start() is called via sessionStore.initStaticInfo() in App.tsx.
