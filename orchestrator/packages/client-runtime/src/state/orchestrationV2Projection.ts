...
export function applyOrchestrationV2ProjectionEvent(
  ...
  switch (event.type) {
    ...
    case "provider-thread.updated": {
      const providerThread = event.payload;
      const queuedPlaceholder = providerThread.status === "not_loaded" && providerThread.firstRunOrdinal === null && providerThread.nativeThreadRef === null && providerThread.providerSessionId === null;
      return {
        ...base,
        // Match the server projector: a newly opened native thread becomes the active continuation.
        thread: providerThread.appThreadId === base.thread.id && !queuedPlaceholder
          ? { ...base.thread, activeProviderThreadId: providerThread.id } : base.thread,
        providerThreads: upsertEntity(base.providerThreads, providerThread),
      };
    }
