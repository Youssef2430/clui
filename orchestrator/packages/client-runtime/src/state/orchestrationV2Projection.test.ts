...
it("tracks active native continuation without letting a queued placeholder replace it", () => {
  const providerThread = {
    id: ProviderThreadId.make("native-ready"), appThreadId: threadId, status: "idle", firstRunOrdinal: 1,
    nativeThreadRef: { driver: "opencode", nativeId: "session-ready", strength: "strong" }, providerSessionId: "session-runtime",
  };
  const event = { id: "native-event", type: "provider-thread.updated", threadId, occurredAt: now, payload: providerThread } as OrchestrationV2DomainEvent;
  const live = applyOrchestrationV2ProjectionEvent(emptyProjection, event)!;
  expect(live.thread.activeProviderThreadId).toBe(providerThread.id);
  const queued = { ...event, payload: { ...providerThread, id: ProviderThreadId.make("queued"), status: "not_loaded", firstRunOrdinal: null, nativeThreadRef: null, providerSessionId: null } } as OrchestrationV2DomainEvent;
  expect(applyOrchestrationV2ProjectionEvent(live, queued)?.thread.activeProviderThreadId).toBe(providerThread.id);
});
