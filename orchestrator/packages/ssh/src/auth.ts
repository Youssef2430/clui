...
# Invoked by ssh via SSH_ASKPASS when GLUI re-runs ssh with a cached password
...
printf 'GLUI ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when GLUI re-runs\r
...
[Console]::Error.WriteLine("GLUI ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
