...
export function resolvePiLaunchArgs(launchArgs: string): PiLaunchArgsResolution {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (reserved !== undefined) {
      return {
        ok: false,
        message: `Pi launch argument '${reserved}' is controlled by GLUI and cannot be overridden.`,
      };
    }
    if (arg === "--") {
      return {
    ...
    if (arg.startsWith("-")) {
      return { ok: false, message: `Pi launch argument '${arg}' is not supported by GLUI.` };
    }
    return {
      ok: false,
