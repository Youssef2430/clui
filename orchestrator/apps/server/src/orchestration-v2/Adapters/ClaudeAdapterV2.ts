...
export const createClaudeAdapterV2 = Effect.fn("ClaudeAdapterV2Driver.create")(
  function* (
    ...
    const binaryPath = yield* resolveClaudeSdkExecutablePath(config.binaryPath, claudeEnvironment);
...
const makeDefaultClaudeAdapterV2 = Effect.fn("ClaudeAdapterV2.layer")(function* () {
  ...
  const serverConfig = yield* ServerConfig;
  const continuationRequests = yield* ProviderContinuationRequests;
  const binaryPath = yield* resolveClaudeSdkExecutablePath(DEFAULT_CLAUDE_SETTINGS.binaryPath, hostEnvironment);
  return makeClaudeAdapterV2({
    instanceId: CLAUDE_DEFAULT_INSTANCE_ID,
    settings: { ...DEFAULT_CLAUDE_SETTINGS, binaryPath },
