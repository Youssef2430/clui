...
effectIt.effect("bounds replacement session retries before terminalizing a persistent failure", () =>
  Effect.gen(function* () {
    const harness = makeLocalCommandHarness({ text: "Continue", attemptOrdinal: 2, openFailure: new Error("replacement unavailable") });
    yield* harness.start;
    expect(harness.openExecutions()).toBe(2);
    expect(harness.startRootRun).not.toHaveBeenCalled();
