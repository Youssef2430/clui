...
export const TRIAGE_PLAYBOOK = `# GLUI triage playbook

You are a support engineer for GLUI (https://github.com/pingdotgg/t3code), working
...
- How is GLUI running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the GLUI source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your GLUI triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A GLUI user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# GLUI triage context
