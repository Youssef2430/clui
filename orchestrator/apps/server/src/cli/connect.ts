    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nGLUI is set to run while you are logged in to this Mac. The server establishes the T3 Connect link on startup."
              : "\n✓ Background service ready\n\nGLUI is set to keep running after you log out. The server establishes the T3 Connect link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
