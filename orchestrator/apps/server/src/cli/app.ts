...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the GLUI desktop app. Start or update the desktop app on this machine, then run `t3 app` again. A running GLUI server is not enough.";
  }
}
...
export class DesktopAppRequestFailedError extends Schema.TaggedError<DesktopAppRequestFailedError>()(
  ...
  override get message(): string {
    return `GLUI could not open ${this.workspaceRoot} (${this.code}).`;
  }
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
  ...
  yield* Console.log(`Opened ${workspaceRoot} in GLUI.`);
});
...
export const appCommand = Command.make("app", {
  ...
  Command.withDescription("Open a project in the running GLUI desktop app."),
