...
export const baseDirFlag = Flag.String("base-dir").pipe(
  Flag.withDescription(
    "Explicit GLUI data directory; runtime state is stored under userdata (equivalent to T3CODE_HOME).",
  ),
  Flag.optional,
);
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    // GLUI's own OTLP variables name no signal, so the one answer they give
