...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install GLUI as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `GLUI service is already installed with t3@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} GLUI service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`GLUI service is already using t3@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} GLUI service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the GLUI service${status.installedVersion === undefined ? "" : ` on t3@${status.installedVersion}`}.`
            : "GLUI service is not installed.",
        );
      }),
    ),
  ),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the GLUI background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the GLUI service." : "GLUI service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the GLUI background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("GLUI is already set up to run in the background on this machine.");
    return true;
  }
  ...
  const wanted = yield* Prompt.run(
    Prompt.Confirm({
      message: installed
        ? "The installed GLUI service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run GLUI in the background whenever you log in to this Mac? " +
            "It stays reachable through T3 Connect while you are logged in."
          : "Run GLUI in the background whenever this machine boots? " +
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the GLUI background service."),
