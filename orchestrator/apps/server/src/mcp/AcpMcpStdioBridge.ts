...
export function callAcpMcpTool(
  ...
  return Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!response.ok) {
          ...
          return yield* Effect.fail(
            new AcpMcpBridgeError(`GLUI MCP endpoint responded with HTTP ${response.status}.`),
          );
        }
    ...
    if (initializeResponse === undefined || asEnvelope(initializeResponse)?.error !== undefined) {
      return yield* Effect.fail(
        new AcpMcpBridgeError("GLUI MCP endpoint rejected initialization."),
      );
    }
    ...
    if (envelope === null || envelope.error !== undefined) {
      return yield* Effect.fail(
        new AcpMcpBridgeError(
          `GLUI MCP tool call failed${envelope?.error === undefined ? "." : `: ${JSON.stringify(envelope.error)}`}`,
        ),
      );
    }
      ...
      Effect.gen(function* () {
        ...
        if (!response.ok) {
          if (envelope.id !== undefined) {
            yield* respondWithError(
              envelope.id,
              `GLUI MCP endpoint responded with HTTP ${response.status}.`,
            );
          }
        ...
        Effect.catchCause((cause) => {
          ...
          return respondWithError(
            envelope.id,
            `GLUI MCP bridge request failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }),
      );
