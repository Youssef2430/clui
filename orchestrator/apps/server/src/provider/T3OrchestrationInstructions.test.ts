...
describe("T3 orchestration provider instructions", () => {
  ...
  it("gives ACP sessions provider-neutral mode, browser, and orchestration guidance", () => {
    ...
    assert.include(injected, "GLUI interaction mode: Default");
    assert.include(injected, "GLUI collaborative browser");
    assert.include(injected, "GLUI orchestration");
  ...
  it("reinjects ACP guidance only when mode or tool availability changes", () => {
    ...
    assert.include(
      ...
      "GLUI interaction mode: Plan",
    );
    const withoutMcp = t3AcpPromptWithInstructions({
      prompt,
      state: { interactionMode: "default", hasT3Mcp: false },
    });
    assert.include(withoutMcp, "GLUI interaction mode: Default");
    assert.notInclude(withoutMcp, "GLUI collaborative browser");
    assert.notInclude(withoutMcp, "GLUI orchestration");
  });
});
