...
describe("resolveClaudeSdkExecutablePath", () => {
  it.effect("resolves bare CLI commands before the SDK checks them on macOS and Linux", () =>
    Effect.gen(function* () {
      for (const platform of ["darwin", "linux"] as const) {
        expect(
          yield* resolveClaudeSdkExecutablePath("claude", { PATH: "/home/test/.local/bin" }).pipe(
            Effect.provideService(HostProcessPlatform, platform),
            Effect.provideService(SpawnExecutableResolution, (command, os, env) => {
              expect(command).toBe("claude");
              expect(os).toBe(platform);
              expect(env.PATH).toBe("/home/test/.local/bin");
              return "/home/test/.local/bin/claude";
            }),
          ),
