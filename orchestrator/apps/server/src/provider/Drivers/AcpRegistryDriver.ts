  ...
  function* (input: SnapshotIdentity & { readonly settings: AcpRegistrySettings }) {
    ...
    return baseSnapshot({
      ...
      message: input.settings.enabled
        ...
        : "ACP Registry is disabled in GLUI settings.",
    });
  },
);
...
export function buildCheckedAcpRegistrySnapshot(
  ...
  const authenticationMessage = advertisedAuthMethod
    ? advertisedAuthMethod.type === "terminal" && advertisedAuthMethod.command
      ? `Run \`${advertisedAuthMethod.command}\` in a thread terminal on this environment. GLUI will detect the completed sign-in on the next provider refresh.`
      : advertisedAuthMethod.type === "env_var" &&
          (advertisedAuthMethod.envVarNames?.length ?? 0) > 0
        ? `Set ${advertisedAuthMethod.envVarNames!.join(", ")} under this instance's environment variables in provider settings. GLUI will detect it on the next provider refresh.`
        : `Complete the advertised "${advertisedAuthMethod.name}" authentication method on the server. GLUI will detect it automatically on the next provider refresh.`
    : undefined;
  return baseSnapshot({
    ...input,
