...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "GLUI (Alpha)",
    linuxDesktopEntryName: "com.clui.GLUI.desktop",
    linuxWmClass: "glui",
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "Name=GLUI (Nightly)");
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "glui",
      desktopEntryPath: "/home/alice/.local/share/applications/com.clui.GLUI.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the glui:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/com.clui.GLUI.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the glui:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "com.clui.GLUI.desktop", "x-scheme-handler/t3code"],
        },
      ]);
    });
  });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(unpackaged, {
        environment: {
          isPackaged: false,
          linuxDesktopEntryName: "com.clui.GLUI.Development.desktop",
        },
      });
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/com.clui.GLUI.Development.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/com.clui.GLUI.desktop",
        }),
      });
