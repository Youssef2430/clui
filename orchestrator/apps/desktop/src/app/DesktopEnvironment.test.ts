...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.clui.app.dev");
      assert.equal(environment.linuxWmClass, "glui-dev");
      assert.equal(environment.linuxDesktopEntryName, "com.clui.GLUI.Development.desktop");
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "com.clui.GLUI.desktop");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.clui.app.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.clui.app.dev.local");
    }),
  );
