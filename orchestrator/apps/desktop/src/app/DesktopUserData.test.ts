...
for (const sourceName of ["glui", "GLUI (Alpha)"]) {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const destination = path.join(directory, "glui-v2");
        const state = '{"os_crypt":{"encrypted_key":"test-encrypted-key"}}';
        yield* fs.makeDirectory(path.join(directory, "GLUI (Alpha)"), { recursive: true });
