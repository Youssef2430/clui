...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    expect(await portal.backend("com.clui.GLUI")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.clui.GLUI")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.clui.GLUI")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    ...
    expect(await updated.backend("com.clui.GLUI")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.clui.GLUI", {
      flash: true,
      animate: true,
    });
    ...
    expect(activateTitle).toBe("GLUI");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.clui.GLUI")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "com.clui.GLUI.NiriTest.SnapShot",
      "--object-path",
    ...
    const invalid = server.call(
      new Message({
        destination: "com.clui.GLUI.NiriTest.SnapShot",
