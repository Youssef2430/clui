...
const MAC_SCREEN_CAPTURE_PERMISSION_MESSAGE =
  "Allow Screen Recording in System Settings, then restart GLUI.";
const MAC_ACCESSIBILITY_PERMISSION_MESSAGE =
  "Allow Accessibility in System Settings, then restart GLUI.";
const MAC_BOTH_PERMISSIONS_MESSAGE =
  "Allow Accessibility and Screen Recording in System Settings, then restart GLUI.";
...
export class DesktopSnapShot extends Context.Service<
  DesktopSnapShot,
  {
    ...
    /** Capture the foreground window in place, including GLUI itself. */
...
export const make = Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (snapshot.linuxActivationFailure) {
        yield* Effect.logWarning(
          "The compositor could not activate GLUI after the snapshot",
  ...
  const checkShortcut = Effect.fn("desktop.snapShot.checkShortcut")(function* (
    ...
    if (mode === "portal" && niriSocketPath()) {
      return {
        available: false,
        message: "Configure the capture shortcut in your Niri config, not in GLUI.",
      };
    }
    ...
    if (mode === "portal" && niriSocketPath()) {
      const registered = yield* Effect.tryPromise(async () => {
        ...
        return startNiriCaptureShortcut(linuxAppId, onCurrentShortcut, () => {
          void runPromise(
            setShortcutFailure("The Niri capture endpoint disconnected. Restart GLUI."),
          ).catch(() => undefined);
        });
      }).pipe(
        Effect.tap((stop) =>
      ...
      yield* Ref.set(stateRef, {
        ...
        shortcutMessage: registered
          ...
          : "Could not start the Niri capture endpoint. Another GLUI instance may be using it.",
        message: null,
      });
      return;
    }
