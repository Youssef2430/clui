...
export async function installGnomeCaptureBundle({ bundle, dataHome }: SetupPaths) {
  ...
  if (existing) {
    ...
    if (installed.version > metadata.version)
      throw new Error("A newer extension is installed. Update GLUI instead of replacing it.");
  }
  ...
  try {
    ...
    if (existing) {
      const backupParent = NodePath.join(dataHome, "glui", "extension-backups");
    ...
    try {
      ...
      if (installed && (!info.state || (info.version && installed.version > info.version.value)))
        return {
          ...
          message:
            "Installed. Save your work, sign out of GNOME and sign back in, then return here to enable the extension. Restarting GLUI alone is not enough.",
        };
      ...
      if (!properties.UserExtensionsEnabled.value)
        return {
          ...
          message:
            "GNOME has disabled user extensions. Turn on Extensions in the GNOME Extensions app, then check again. GLUI will not enable your other extensions for you.",
        };
      if (info.state?.value === 1)
        return {
          status: "enabled",
          message: "The GLUI extension is running. Active-window snapshots are available.",
        };
      ...
      return {
        status: "disabled",
        message:
          "Enable the GLUI extension to allow active-window snapshots. You can disable it here at any time.",
      };
    } catch (error) {
      return {
        status: "error",
