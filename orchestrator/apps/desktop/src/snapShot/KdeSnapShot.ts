...
const DESKTOP_FILE = "com.clui.GLUI.KdeCapture.desktop";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "glui", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall GLUI.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall GLUI.",
        );
