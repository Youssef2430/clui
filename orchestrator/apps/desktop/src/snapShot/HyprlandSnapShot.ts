...
export function hyprlandCaptureExecutable(paths: HyprlandCapturePaths) {
  return NodePath.join(paths.dataHome, "glui", "hyprland-capture", HYPRLAND_CAPTURE_EXECUTABLE);
}
...
export class HyprlandCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        throw new Error(
          "The Hyprland capture helper is missing from this build. Update or reinstall GLUI.",
        );
