...
describe("ElectronProtocol", () => {
  ...
  it.effect("serves the bundled client from disk without a backend", () =>
    Effect.gen(function* () {
      ...
      const request = (pathname: string, init?: RequestInit) =>
        Effect.promise(() => handler!(new Request(`glui://app${pathname}`, init)));
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("glui-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "glui-dev://app",
                  referer: "glui-dev://app/",
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["glui-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("glui://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("glui-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "glui-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(error.message, 'Failed to register Electron protocol scheme "glui-dev".');
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "glui");
        assert.strictEqual(error.cause, cause);
        assert.equal(error.message, 'Failed to unregister Electron protocol scheme "glui".');
      }
    }).pipe(Effect.provide(protocolLayer)),
  );
