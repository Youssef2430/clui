...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/GLUI (Nightly).app/Contents/MacOS/GLUI",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/GLUI.app/Contents/MacOS/GLUI")).toBe(
      "/Applications/GLUI.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/GLUI.app/other/MacOS/GLUI")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/GLUI (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/GLUI (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged GLUI icon is missing");
  expect(windows).toHaveLength(0);
});
