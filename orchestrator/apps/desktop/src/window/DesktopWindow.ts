import { isPillHost, isPillHostQuitting } from "../gluiPillHost.ts";
...
export const make = Effect.gen(function* () {
  ...
  > {
    ...
    window.on("close", (event) => {
      if (isPillHost && !isPillHostQuitting()) { event.preventDefault(); window.hide(); }
    ...
    bindFirstRevealTrigger(revealSubscribers, () => {
      if (isPillHost) { void runPromise(dismissConnectingSplash); return; }
