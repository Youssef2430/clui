...
// Restricted request/reply channel to the owning GLUI pill process.
contextBridge.exposeInMainWorld("gluiPillHost", {
  onRequest: (listener: (request: import("@t3tools/shared/gluiPill").PillRequest) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, request: import("@t3tools/shared/gluiPill").PillRequest) => listener(request);
    ipcRenderer.on("glui:host-request", handler);
    return () => ipcRenderer.removeListener("glui:host-request", handler);
  },
  reply: (reply: import("@t3tools/shared/gluiPill").PillReply) => ipcRenderer.send("glui:host-reply", reply),
});
