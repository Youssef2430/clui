// This file mostly exists because we want dev mode to say "GLUI (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "GLUI (Dev)" : "GLUI (Alpha)";
const APP_BUNDLE_ID = isDevelopment
  ? `com.clui.app.dev.${devBundleIdSuffix || "local"}`
  : "com.clui.app";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["glui-dev"] : ["glui"];
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSDocumentsFolderUsageDescription: "GLUI reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the GLUI name.
