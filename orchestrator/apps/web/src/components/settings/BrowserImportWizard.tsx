...
function FullDiskAccessStep({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Let GLUI read {source.name}&rsquo;s cookies</DialogTitle>
        <DialogDescription>
          To import cookies from {source.name}, GLUI needs Full Disk Access. Turn it on in System
      ...
      <DialogPanel>
        ...
        {!permission.isReady(["fullDiskAccess"]) ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {stillRequired
              ? "Access is still required. Quit and reopen GLUI if you just allowed it, then retry the import."
              : "If access doesn't update after you allow it, quit and reopen GLUI, then retry the import."}
          </p>
        ) : null}
      </DialogPanel>
      <DialogFooter>
