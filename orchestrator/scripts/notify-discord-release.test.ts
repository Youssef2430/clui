...
it("builds a prerelease Discord announcement for nightly subscribers", () => {
  assert.deepStrictEqual(
    buildDiscordReleaseAnnouncement({
      ...
      releaseName: "GLUI Nightly 1.2.4-nightly.20260501.17 (abcdef123456)",
    ...
    {
      content:
        "<@&111111111111111111> Prerelease published: GLUI Nightly 1.2.4-nightly.20260501.17 (abcdef123456)",
      allowed_mentions: {
        roles: ["111111111111111111"],
      },
      embeds: [
        {
          title: "GLUI Nightly 1.2.4-nightly.20260501.17 (abcdef123456)",
          url: "https://github.com/t3dotgg/t3-code/releases/tag/v1.2.4-nightly.20260501.17",
          description: "A new GLUI prerelease is available for nightly testers.",
          color: 0x5865f2,
          fields: [
            {
              name: "Version",
...
it("builds a latest Discord announcement for stable subscribers", () => {
  assert.deepStrictEqual(buildDiscordReleaseAnnouncement(latestAnnouncement), {
    content: "<@&222222222222222222> Latest published: GLUI v1.2.3",
    allowed_mentions: {
    ...
    embeds: [
      {
        ...
        description: "A new GLUI latest release is available.",
        color: 0x2ecc71,
        fields: [
          {
            name: "Version",
