...
export function npmPlatformPackageManifest(
  ...
  return {
    ...
    version,
    description: `GLUI CLI executable for ${platformKey}`,
...
export function npmPlatformPackageReadme(platformKey: CliArchivePlatformKey): string {
  return [
    ...
    "",
    `The GLUI CLI executable for ${platformKey}. Do not install this package directly:`,
...
export function npmLauncherPackageManifest(
  ...
  return {
    ...
    version,
    description: "GLUI CLI. Installs the self-contained executable for this platform.",
    license: serverPackageJson.license,
    repository: serverPackageJson.repository,
    bin: { t3: "./bin/t3.js" },
    files: ["bin", "dist"],
...
try {
  ...
  process.stderr.write(
    [
      "t3: no GLUI CLI build is available for this platform (" + key + ").",
