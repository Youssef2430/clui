...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // agent working inside GLUI), these leak through and the child server
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.String("home-dir").pipe(
    Flag.withDescription(
      "Explicit GLUI data directory; runtime state is stored under userdata (equivalent to T3CODE_HOME). Inside a git worktree this defaults to that worktree's own .t3 so dev state stays off the shared home.",
    ),
    Flag.optional,
