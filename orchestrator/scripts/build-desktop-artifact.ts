...
const DESKTOP_APP_ID = "com.clui.app";
...
export const DESKTOP_FILE_EXCLUSIONS = [
  ...
  "!apps/desktop/prod-resources/cursor-sdk/**/*",
  // GLUI always passes the user's installed Claude executable to the SDK,
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "GLUI (Nightly)"
    : (desktopPackageJson.productName ?? "GLUI");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "GLUI-${version}-${arch}.${ext}",
    ...
    ...(platform === "win"
      ...
      : { asarUnpack: ["node_modules/onnxruntime-node/**/*", "pill/resources/native/*.node"] }),
    extraResources: [
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSMicrophoneUsageDescription: "GLUI uses your microphone to transcribe voice input locally with Whisper.",
        NSScreenCaptureUsageDescription:
          "GLUI captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "GLUI",
          schemes: ["glui", "glui-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      ...
      // glui:// OAuth callbacks to the app.
      protocols: [
        {
          name: "GLUI",
          schemes: ["glui", "glui-dev"],
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const configuredMacPasskeySigning =
    options.platform === "mac" && options.signed && !(yield* Config.String("GLUI_PILL_ROOT").pipe(Config.option, Effect.map(Option.isSome)))
      ? yield* Effect.try({
  ...
  const windowsServerAsarPath =
    options.platform === "win"
      ...
      : undefined;
  // The default process is GLUI's floating pill. The same signed executable
  // starts this workspace only when its owning pill supplies the private IPC flag.
  const pillRootOption = yield* Config.String("GLUI_PILL_ROOT").pipe(Config.option);
  const pillRoot = Option.getOrUndefined(pillRootOption);
  if (pillRoot) {
    yield* fs.copy(path.join(pillRoot, "dist"), path.join(stageAppDir, "pill/dist"));
    yield* fs.copy(path.join(pillRoot, "resources"), path.join(stageAppDir, "pill/resources"));
    yield* fs.writeFileString(path.join(stageAppDir, "main.cjs"),
      'if (process.env.GLUI_WORKSPACE_HOST === "1" && typeof process.send === "function") require("./apps/desktop/dist-electron/main.cjs"); else require("./pill/dist/main/index.js");\n');
  }
  const stagePackageJson: StagePackageJson = {
    ...
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "GLUI desktop build",
    author: "T3 Tools",
    main: pillRoot ? "main.cjs" : "apps/desktop/dist-electron/main.cjs",
  ...
  for (const entry of stageEntries) {
    ...
    if (stat.type === "Directory" && (options.target === "dir" || (options.platform === "mac" && /^mac(?:-arm64|-x64)?$/.test(entry)))) {
      const to = path.join(options.outputDir, entry);
      yield* fs.remove(to, { recursive: true, force: true });
      // Preserve framework-relative links; fs.copy rewrites them to the temporary stage.
      yield* Effect.tryPromise(() => NodeFSP.cp(from, to, { recursive: true, force: true, verbatimSymlinks: true }));
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for GLUI."),
